...
// ===== URL 파라미터로 자동 행 추가 =====
// management.html에서 "항목 관리" 링크 클릭 시 넘어오는 데이터 처리
...
function saveAll() {
    saveRows(rows);
    const validCount = rows.filter(r => r.category && r.line && r.target && r.point && r.items.length > 0).length;
